...
export const CartVariants = tv({
  slots: {
    ...
    cartLabel: 'absolute top-3 left-px px-4 font-medium bg-in-slate text-in-cyan peer-focus:text-in-cyan peer-placeholder-shown:text-in-stone peer-invalid:text-in-red peer-focus:peer-invalid:text-in-red scale-75 peer-focus:scale-75 peer-placeholder-shown:scale-100 transform -translate-y-5 peer-focus:-translate-y-5 peer-placeholder-shown:translate-y-0 transition ease-in-out duration-300',
    cartError: 'absolute -bottom-2 right-2 px-4 bg-in-slate font-medium text-in-orange text-xs',
    cartPay: 'w-full flex items-center justify-center px-4 h-12 uppercase cursor-pointer rounded-md border border-in-cyan bg-transparent ui-checked:bg-in-cyan/5 hover:bg-in-cyan/5 ui-checked:text-in-cyan transition ease-in-out duration-300',
...
export const ModalVariants = tv({
  slots: {
    ...
    modalBackdrop: 'fixed inset-0 bg-black/10',
    modalDialog: 'fixed inset-0 overflow-y-auto',
    modalInfo: 'flex min-h-full items-center justify-center text-center p-4',
    modalPanel: 'w-full max-w-xl flex flex-col items-center transform overflow-hidden rounded-md shadow-lg bg-in-white text-in-dark p-6 align-middle transition-all',
    modalIcon: 'animate-bounce size-12 shrink-0 text-in-cyan',
    modalTitle: 'text-xl font-medium leading-6 mt-2',
    modalDescription: 'mt-4',
    modalSumary: 'flex flex-col gap-4 font-medium text-sm mt-10',
    modalAction: 'mt-10 w-full flex items-center justify-center px-4 h-12 font-medium uppercase rounded-md bg-in-cyan text-in-stone transform active:scale-95 hover:-translate-y-1 transition ease-in-out duration-300',
